...
test.describe('Filters and Search', () => {
  ...
  test('should filter records by status showing only pending records', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="status-badge-record-1"]')).toContainText('Pending');
    await expect(page.locator('[data-testid="status-badge-record-3"]')).toContainText('Pending');
  });

  test('should filter records by source type showing only forms', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="record-row-record-1"]')).toBeVisible();
  });

  test('should search records by query and filter results', async ({ page }) => {
  ...
  test('should combine filters and search to narrow results', async ({ page }) => {
